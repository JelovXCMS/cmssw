...
#ifndef akSoftDrop4PFJetClass_h
#define akSoftDrop4PFJetClass_h
...
#ifdef akSoftDrop4PFJetClass_cxx
akSoftDrop4PFJetClass::akSoftDrop4PFJetClass(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("Test_HiForestAOD_pp_MC_PYTHIA6_bjet30.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("Test_HiForestAOD_pp_MC_PYTHIA6_bjet30.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("Test_HiForestAOD_pp_MC_PYTHIA6_bjet30.root:/akSoftDrop4PFJetAnalyzer");
...
akSoftDrop4PFJetClass::~akSoftDrop4PFJetClass()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t akSoftDrop4PFJetClass::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t akSoftDrop4PFJetClass::LoadTree(Long64_t entry)
{
...
void akSoftDrop4PFJetClass::Init(TTree *tree)
{
...
Bool_t akSoftDrop4PFJetClass::Notify()
{
...
void akSoftDrop4PFJetClass::Show(Long64_t entry)
{
...
Int_t akSoftDrop4PFJetClass::Cut(Long64_t entry)
{
...
#endif // #ifdef akSoftDrop4PFJetClass_cxx
