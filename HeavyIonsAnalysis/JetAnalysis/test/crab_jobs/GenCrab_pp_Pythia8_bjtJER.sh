...
cat > crab_${INFILENAME}_BjtJER.py <<EOF


from CRABClient.UserUtilities import config, getUsernameFromSiteDB
config = config()

config.General.requestName = '${INFILENAME}_HiForest75X_BjtJER'
config.General.workArea = 'crab_projects'
config.General.transferOutputs = True
config.General.transferLogs = False

config.JobType.pluginName = 'Analysis'
config.JobType.psetName = '../runForestAOD_pp_MC_75X_BjtJER.py'
...
config.Data.outputDatasetTag = '${INFILENAME}_HiForest75X_BjtJER'
...
crab submit crab_${INFILENAME}_BjtJER.py
