...
class TkAlDiMeson : public edm::EDAnalyzer {
 public:
  ...
  // int Z_idx_daughter1[max_Z];
  // int Z_idx_daughter2[max_Z];
 /* double mass_daughter1[max_XB];
  double phi_daughter1[max_XB];
  double pt_daughter1[max_XB];
  double eta_daughter1[max_XB];
  double charge_daughter1[max_XB];
  double mass_daguther2[max_XB];
  double phi_daguther2[max_XB];
  double pt_daguther2[max_XB];
  double eta_daguther2[max_XB];
*/

/*
  double Track_dxyBS[max_XB];
  double Track_d0BS[max_XB];
  double Track_dzBS[max_XB];
  double Track_dxyPV[max_XB];
  double Track_d0PV[max_XB];
  double Track_dzPV[max_XB];

  double Track_mass[max_XB];
  double Track_phi[max_XB];
  double Track_eta[max_XB];
  double Track_pt[max_XB];
  double Track_charge[max_XB];
*/  
  //=====================================================
  ...
  virtual void analyze(const edm::Event& event, const edm::EventSetup& setup){
    ...
    memset(&MuMu_mass, 0x00, sizeof(MuMu_mass));
    memset(&MuMu_pt, 0x00, sizeof(MuMu_pt));
    memset(&MuMu_phi, 0x00, sizeof(MuMu_phi));
    memset(&MuMu_eta, 0x00, sizeof(MuMu_eta));
    // memset(&MuMu_idx_daughter1, 0x00, sizeof(MuMu_idx_daughter1));
    // memset(&MuMu_idx_daughter2, 0x00, sizeof(MuMu_idx_daughter2));
    // memset(&Track_mass,0x00, sizeof(Track_mass));
    // memset(&Track_pt,0x00, sizeof(Track_pt));
    // memset(&Track_phi,0x00, sizeof(Track_phi));
    // memset(&Track_eta,0x00, sizeof(Track_eta));
    ...
    for (reco::TrackCollection::const_iterator track=tC.begin(); track!=tC.end(); track++){
      ...
      // i_daughter1++;
      // Track_mass[i_Track]=0.105;
      // Track_pt[i_Track]=track0.Pt();
      // Track_phi[i_Track]=track0.Phi();
      // Track_eta[i_Track]=track0.Eta();
      // i_Track++;
      ...
      for (reco::TrackCollection::const_iterator track1=track+1; track1!=tC.end(); track1++){
        ...
        // i_daughter2++;
				...
				if(mother.M()>0 && mother.M()<150){

        MuMu_mass[MuMu_size]=mother.M();
        MuMu_pt[MuMu_size]=mother.Pt();
        MuMu_phi[MuMu_size]=mother.Phi();
        MuMu_eta[MuMu_size]=mother.Eta();
        ...
        SMuMu_mass=mother.M();
        SMuMu_pt=mother.Pt();
        SMuMu_phi=mother.Phi();
        SMuMu_eta=mother.Eta();
				STrk1_pt=ptMu1;
				STrk1_eta=etaMu1;
				STrk1_phi=phiMu1;
				STrk2_pt=ptMu2;
...
if(MuMu_size>0 && DEBUG){
  ...
  virtual void beginJob() {
    ...
    myTree->Branch("MuMu_size",&MuMu_size,"MuMu_size/I");
    myTree->Branch("MuMu_mass",MuMu_mass,"MuMu_mass[MuMu_size]/F");
    myTree->Branch("MuMu_pt",MuMu_pt,"MuMu_pt[MuMu_size]/F");
    myTree->Branch("MuMu_phi",MuMu_phi,"MuMu_phi[MuMu_size]/F");
    ...
    DiMuTree->Branch("MuMu_pt",&SMuMu_pt,"SMuMu_pt/F");
    DiMuTree->Branch("MuMu_phi",&SMuMu_phi,"SMuMu_phi/F");
    DiMuTree->Branch("MuMu_eta",&SMuMu_eta,"SMuMu_eta/F");
...
//    myTree->Branch("MuMu_idx_daughter1",&MuMu_idx_daughter1,"MuMu_idx_daughter1");
//    myTree->Branch("Z_idx_daughter2",&Z_idx_daughter2,"Z_idx_daughter2");
//    myTree->Branch("Track_mass",&Track_mass,"Track_mass");
//    myTree->Branch("Track_pt",&Track_pt,"Track_pt");
//    myTree->Branch("Track_phi",&Track_phi,"Track_phi");
//    myTree->Branch("Track_eta",&Track_eta,"Track_eta");
